...
def create_copBB_code(ell, m, p1, p2, name=None):
    # l,m are two primes
    # p1 = (a,b,c), p2 = (d,e,f)
    # A = \sum xy^p1[i]
    # B = \sum xy^p2[i]
    ...
    return css_code(hx, hz, name=name, name_prefix="copBB", check_css=True)

'''
def coPBB_code(ell, m, p1, p2):
    I_ell = np.identity(ell, dtype=int)
    I_m = np.identity(m, dtype=int)
    x = np.kron(np.roll(I_ell, 1, axis=1), I_m)
    y = np.kron(I_ell, np.roll(I_m, 1, axis=1))
    ...
    A_list = [np.linalg.matrix_power(pi, i) % 2 for i in p1]
    B_list = [np.linalg.matrix_power(pi, i) % 2 for i in p2]
    A = sum(A_list) % 2
    B = sum(B_list) % 2
